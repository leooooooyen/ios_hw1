...
func distinctAuthors() -> [String] {
    ...
    for index in 0...books.count-1{
        if someauthors.isEmpty{
            someauthors.append(books[index]["author"]!)
        }
        else if someauthors[count] != books[index]["author"]{
            someauthors.append(books[index]["author"]!)
            count += 1
        }
    }
...
func totalBookPrice() -> Double {
    var total = 0.0
    for index2 in 0...books.count-1{
        total += Double(books[index2]["price"]!)!
    }
    return (total)
}
